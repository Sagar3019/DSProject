...
<?php 
	...
	<div id="content">
	  	<?php
			...
			$sql="use books";
			$con->query($sql);
			$sql="select Title,Author,Quantity,Img_Path,Description from me";
			$qr=$con->query($sql);
			if($qr->num_rows > 0) 
			{
  			  while($res = $qr->fetch_array()) 
  			  {
				echo ('<div class="book"> <img src="').$res["Img_Path"].('" alt="').$res["Title"].(' by ').$res["Author"].('" width="230px" height="250px"> <form action="extended Level 2 me.php" method="POST">  <input type="submit" name="title" value="').$res["Title"].('">  </form> </div>');
			}
		  }	
		?>
	</div>
