...
<?php 
	...
	<div id="content">
		<?php
			...
			$sql="use books";
			$con->query($sql);
			$sql="select Title,Author,Quantity,Img_Path,Description from cse where Title='$title'";
			$qr=$con->query($sql);
			if($qr->num_rows > 0) 
			{
		   ...
		   {
				echo('<form action="extended level 3.php" method="POST">
			...
			<b>Book Description :</b> ') .$res["Description"].('</p>');
				...
				$con->query($sql);
				echo "<center><strong id='pve'>This Book Has Been Issued to ".$_COOKIE["login"]." for 15 day. Please collect from Library!!</strong></center>";
		}
	?>
	</div>
</body>
